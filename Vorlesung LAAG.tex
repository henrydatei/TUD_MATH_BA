...
\section{Vektorräume}
	...
	\subsection{Summen von Vektorräumen}
		...
		\begin{framed}
			...
			darstellbar,m so sagt man, dass $\sum W_i$ die direkte Summe der UVR $W_i$ ist und schreibt $\oplus W_i$ für 
...
\section{Lineare Abbildungen}
	...
	\subsection{Matrizen}
		...
		\begin{framed}
			...
			$c_{ik}=\sum\limits_{j=1}^n a_{ij}\cdot b_{jk}$. Kurz geschreiben "Zeile $\cdot$ Spalte".
		\end{framed}
	...
	\subsection{Homomorphismen von Vektorräumen}
		...
		\begin{framed}
			\textbf{Definition $K$-linear:} Eine Abbildung $f: V \to W$ heißt $K$-linear Homomorphismus von $K$-VR, wenn für 
			alle $x,y\in V$ und $\lambda\in K$ gilt: \\
			(L1): $f(x+y)=f(x)+f(y)$ \\
			(L2): $f(\lambda x)=\lambda \cdot f(x)$ \\
			Die Menge der $K$-linearen Abbildungen $f: V\to W$ wird mit $Hom_K(V,W)$ bezeichnet. Die Elemente von $End_K(V,W)
		...
		\textbf{Bemerkung:} Eine $K$-lineare Abbildung $f: V\to W$ ist also ein Homomorphismus der abeslchen Gruppen $(V,+) 
		...
		Ausgeschrieben: Ist $A=(a_{ij})$ und $x=(x_1,...,x_n)^t$ so ist $f_A=Ax=
		\begin{pmatrix}
		a_{11} & ... & a_{1n}\\
		... &  & ...\\
		a_{m1} & ... & a_{mn}\\
	...
	\subsection{Der Vektorraum der linearen Abbildungen}
		...
		\begin{framed}
			...
			V$ mit $f(e_i)=v_i$. Insbesondere ist jeder endlichdimensionale $K$-VR zu einem Standardraum isomorph, nämlich zu
			$K^n$ für $n=dim_K(V)$.
		\end{framed}
		...
		\begin{framed}
			...
			Abbildungn $f_A\in Hom_K(K^n,K^m)$, $f_B\in Hom_K(K^r,K^n)$ gilt dann $f_{AB}=f_A\circ f_B$.
		\end{framed}
		\textit{Beweis: \\
