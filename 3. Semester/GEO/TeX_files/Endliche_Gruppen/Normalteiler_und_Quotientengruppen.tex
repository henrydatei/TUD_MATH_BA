...
\begin{definition}[normal, Normalteiler]
	Eine Untergruppe $H\le G$ ist \begriff{normal} (in Zeichen $H\unlhd G$), wenn $g^{-1}hg\in H$ für alle $h\in H$ und $g\in G$. Ein \begriff{Normalteiler} von $G$ ist (auch eine) Untergruppe von $G$.
\end{definition}
...
\begin{lemma}
	...
	\begin{enumerate}[label=(\alph*)]
		\item $N\unlhd G\Leftrightarrow gN=Ng$ für alle $g\in G$
...
\begin{proof}
	\begin{enumerate}[label=(\alph*)]
		\item Hinrichtung: $\forall g\in G$, $\forall n\in N$: $g^{-1}ng\in N\Rightarrow gNg^{-1}\subseteq N\Rightarrow Ng=gN$ und $g^{-1}N\subseteq Ng^{-1}\Rightarrow gN=Ng$ \\
		Rückrichtung: $\forall g\in G$: $gN=Ng \Rightarrow\exists n'\in N$: $gn'=ng\Rightarrow g^{-1}ng=n\in N$
		\item \begin{itemize}
			\item $HN=\bigcup_{h\in H} hN=\bigcup_{h\in H} Nh=NH$. Alternative: Sei $HN = \set{hn \mid h \in H, n \in N}$ und $NH = \set{nh \mid n\in , h\in H}$. Seien $n \in N$ und $h \in H$.
			Wir haben zwei Inklusionen zu zeigen:
			\begin{itemize}
				\item $HN\subset NH$: Wenn $n \in N, h \in H$, dann haben wir $HN \ni hn = hne = hnh^{-1}h \in NH$, da $N$ normal ist.
				\item $HN\subset NH$: Wenn $n \in N, h \in H$, dann haben wir $HN \ni nh = ehn = hh^{-1}nh \in HN$, da $N$ normal ist.
			\end{itemize}
			Damit folgt dann die geforderte Gleichheit.
			\item $HN \le G$: Müssen die (Unter-) Gruppendefinition abprüfen: 
				\begin{itemize}
					\item Wir haben $HN \subset G$ klar.
					\item Seien $h_1,h_2 \in H$, $n_1, n_2 \in N$, dann folgt $h_1 n_1 h_2 n_2 = h_1 h_2 (h_2^{-1} n_1 h_2)n_2 \in N$.
					\item $1 \in HN$ klar
					\item Seien $h \in h, n \in N$, dann folgt $hn \in HN$ und wir haben $(hn)^{-1} = n^{-1}h^{-1} \in NH = HN$.
				\end{itemize}
			Also gilt $HN \le G$.
...
\begin{lemma}
	...
	\begin{align*}
		H\le G \text{ mit }N\le H \leftrightarrow H\le\lnkset{G}{N}
	\end{align*}
\end{lemma}
\begin{proof}
	\begin{itemize}
...
\begin{proof}
	...
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
	Dieser hat Kern $\Ker(\overline{\pi_H})=\lnkset{H}{N}$, induziert nach \propref{1_3_9} einen Isomorphismus %TODO wahts the ``?'' doing here?! maybe a ``=''?
	\begin{align}
