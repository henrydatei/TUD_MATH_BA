...
\begin{enumerate}
	...
	A \begriff{stochastic process} $S$ is a measurable mapping $S: (\O,\F) \to \R^d \mit (\omega, t) \mapsto S_t(\o)$\\
	Especially:
	\begin{itemize}
		\item $t \mapsto S_t(\o)$ function $I \to \R^d$ for every $\omega \in \O$ (``path'')
	...
	\begin{*example}
		A stochastic process  $(S_t)_{t\in I}$ on $(\Omega,F)$ is called \begriff[stochastic process]{adaptiert} regarding a percolation $(\F_t)_{t \in I}$, wenn gilt:
		\begin{align*}
