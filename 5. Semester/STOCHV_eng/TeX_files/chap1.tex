...
\begin{enumerate}
	\item \term{Bond}: (more precisely: zero-coupon bond) Der \term{issuer} of a bond with maturity $T$ guaranties to pay the buyer the amount $N$ (EUR/USD/...) at time $T$.\\
	Typical issuers:
	\begin{itemize}
	...
	Price at time $t$: $F_t$
	\item \term{European Put-/Call-Option}:
	...
	Time $\tau$ must generally be determined as a solution of a stochastic optimization problem (``\term{Optimal stop problem}'')
\end{enumerate}
...
We use: 
\begin{itemize}
	\item \term{Replication principle}: Two identical future cash flows have the same value even today. (A cash flow ``replicates'' the other) % but what about inflation? Where comes inflation in this whole system in? 
	\item \term{No-arbitage-principle}: ``Without capital investment, secure profit can be achieved without risk of loss''
	\item \term{Arbitrage}: risk-free profit\\
