...
\begin{definition}[Ordnung]
	\begin{itemize}
		\item Ist $G$ endlich, so nennt man $\abs{G}$ die \begriff{Ordnung} von $G$.
...
\begin{definition}[Konjugationsklasse]
	Die Menge $\Cl(y) = \set{x^{-1}yx \mid x \in G}$ ist i.A. \emph{keine} Untergruppe und diese nennt man \begriff{Konjugationsklasse} von $y$. Die \begriff{Klassennummer} von $G$ nennt man die Anzahl der nicht-äquivalenten Konjugationsklassen.
\end{definition}
...
\begin{definition}[normale Untergruppe]
	Eine Untergruppe $H < G$ ist \begriff[Gruppe]{normal} $\Leftrightarrow$ $\forall x \in G\colon xH = Hx$. Notation: $H \lhd G$.
\end{definition}
\begin{proposition}
	\begin{align*}
...
\begin{proof} % fix label!!!
	...
	\item Im letzten Schritt muss noch gezeigt werden, dass $\bar{f}$ ein Gruppenhomorphismus ist: $\bar{f}(x \ker f \cdot y \ker f) = \bar{f}(xy \cdot \ker f)$ (\propref{1_4_3_prop}). $\bar{f}(x \ker f)\bar{f}(y\ker f) = f(x)f(y) = f(xy)$. Also ist $\bar{f}$ ein Morphismus von Gruppen.
	\end{itemize}
\end{proof}
\begin{example}
...
\end{definition}
\begin{*example}
	$G = \Z_p,p$ prim, da es keine \emph{echten} Untergruppen gibt. (\propref{def_p-group})
\end{*example}
\begin{proposition}
...
\begin{*example}
	...
	\begin{align*}
		H = \set{1, r^3} \cong \Z_2 \und K = \set{s^j, r^{2i} \mid i =0,1,2; j=0,1} \cong D_3 \nd D_6 \cong \Z_2 \times D_3
	\end{align*}
\end{*example}
...
\begin{example}
	...
	\begin{align*}
		\abs{\lnkset{A_4}{N}} = \frac{\abs{A_4}}{\abs{N}} = \frac{12}{4} = 3 \quad \text{ Lagrange Theorem} \implies \lnkset{A_4}{N} \cong \Z_3 = \lnkset{\Z}{3\Z} \text{ einfach.}
	\end{align*}
	\begin{align*}
		G_3:= \set{1}\\
...
\begin{proof}
	...
	\begin{itemize} %TODO restructurize ...
...
\begin{definition}[semidirekte Produkt]
	Das \begriff{semidirekte} Produkt (äussere) zweier Gruppen $K,L$ bezüglich eines Homomorphismus
	\begin{align*}
		\rho\colon L \to \Aut(K)
	\end{align*}
...
\end{example}
 Noch allgemeiner:\\
$G$ Gruppe, $G = KL, K \cap L = \set{1}$, d.h. alle $g \in G$ können eindeutig als $xy = y$ mit $x \in K,y\in L$ geschrieben werden, aber $K\not \lhd G, L \not \lhd G$. $G$ nennt man dann ein Bieressprodukt (\person{Zappa}-\person{Zsep}-Produkt, exakt Faktorisierung) (innere Version). $K,L$ matched pair of groups (äussere Version).
\begin{example}[\person{Iwasawa}zerlegung]
	\begin{align*}
