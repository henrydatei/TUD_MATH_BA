...
\begin{definition}[Fahne]
	\begin{enumerate}
		\item Eine \begriff{Fahne} in einem Vektorraum ist eine Folge von Untervektorräumen
		\begin{align*}
		0 = V_0 \subseteq V_1 \subseteq \cdots \subseteq V_d = V \mit \dim V_i = i
		\end{align*}
		\item Allgemein definiert man \begriff[Fahne]{partielle Fahne}
		\begin{align*}
...
\begin{example}
	\begin{enumerate}
		...
		\end{align*}
		\item insbesondere $R = 1$, dann $F(i_R)$ ist Menge der $i_R$-dim Unterverktorräume von $\F^d$ und das führt auf die \person{Grassmann}varietät $\Grass(i_R,d)$ (Grassmanian)
	...
	\end{enumerate}
\end{example}
Ein Computertomographie liefert eine Funktion $\R P^1 \to \R$ nämlich zu jeder Geraden die Absorption entlang dieser, Geraden und \person{Radon} hatte sich seit 1917 überlegt, wie man die ursprüngliche Absorption $a\colon D \to \R$ zurückberechnet.
\begin{align*}
	a \colon D \to \R,\quad f\colon \R P^1 \to \R \text{,wobei } f(L) = \int_L a(x,y) \d x\d y \quad f := R(a)
\end{align*}
das ist die \begriff{Radontransformation}.
\begin{example}
	Sei $d=2, \F = \C$, dann
	\begin{align*}
...
\end{align*}
definiere Felder in solchen Vektorbündel. Diese Felder werden in der Physik (Standardmodell der Elementarteilchen) zur Modellierung von Materie verwendet. Die Krümmung bzw. Wirkung des Bundels modelliert Kräfte (Bosonen z.B. elektromagnetischer Felder). Zur Behandelung des einfachsten nichttrivialsten Beispiel $\R^n, H = \SU(2,\C)$ ``\person{Yang}-\person{Mills}-Theorie'' all das und viel mehr, $\R^k \subseteq S^4 \to \Grass(2,4) = \SU(4,\C)/H$.
\begin{align*}
