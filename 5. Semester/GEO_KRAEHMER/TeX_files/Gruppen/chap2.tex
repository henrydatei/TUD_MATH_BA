...
\end{definition}
\begin{*example}
	$\Z \Circlearrowleft \Z_n, n \neq 0, \overline{x} \in \Z_n$, $g\overline{x} = \overline{g+x}$ nicht treu, da
	\[
	\begin{tikzcd}
	G \arrow[d] \arrow[r, "\theta"] & S_X \arrow[ld, "\overline{\theta}", dashed] \\
	\lnkset{G}{\ker \theta}            &                                            
	\end{tikzcd}
	\]
\end{*example}
\section{Transitive Operationen}
\begin{definition}
	$G \Circlearrowleft X$ \begriff{transitiv}, wenn es genau eine Bahn gibt
	\begin{align*}
		X = G \cdot x_0 \quad x_0 \in X.\\
		\forall x,y \in X\colon \exists g \in G \mit y = gx
	\end{align*}
\end{definition}
\begin{*example}
	Betrachte 
	...
	\end{align*}
	also $O(n) \Circlearrowleft S^{n-1} = \set{x \in \R^n \mid \norm{x} = 1}$ (Drehungen und Spiegelungen in der $S^2$ zum Beispiel) transitiv.
\end{*example}
\begin{lemma}
