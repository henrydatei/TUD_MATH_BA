...
\end{example}
\begin{definition}
	$R$ ist ein HIR $\Leftrightarrow$ $R$ ist \begriff{Integritätsbereich} (eng. integral domain, nullteilerfrei) (ID) und jedes Ideal $I \lhd R$ ist ein Hauptideal, d..h. es existiert $a \in R$ mit
	\begin{align*}
...
\begin{definition}
	Sei $R$ ID, $V$ $R$-Modul
	\begin{enumerate}
		\item $x \in V$ ist ein \begriff{Torsionselement} $\Leftrightarrow \exists r \in R, r \neq 0, r\cdot x = 0$
		\item $T(V) = \set{x \in V \mid x \text{ ist Torsionselement}} \le V$
		\item $V$ ist \begriff{torsionsfrei}, wenn $T(V) = 0$
		\item \begriff{Annihilator} $\Ann_R (V) := \set{r \in R \mid rx = 0 \forall x \in V} \lhd R$
	\end{enumerate}
\end{definition}
\begin{example}
	\begin{enumerate}
...
\begin{proof}
	...
	\begin{itemize}
		...
		\end{align*}
		Also ist $\lnkset{V}{G}$ ein Torsionsmodul ($\lnkset{W}{G}$ Torsion, $\lnkset{V}{W}$ und \propref{2_2_1}) und damit haben wir $F = V$.
