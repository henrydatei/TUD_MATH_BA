...
\begin{definition}[Erwartungsnutzen]
	Sei $U\colon \R \to [-\infty, \infty)$ eine monotone steigende und konkave (``\person{Bernoulli}sche Nutzenfunktion''). Dann definiert
	\begin{align*}
...
\begin{*example}
	...
	\end{itemize}
\end{*example}
\begin{theorem}[Satz von \person{von Neumann} und \person{Morgenstern}, 1953] %4.5
...
\begin{*remark}
	\begin{itemize}
		...
		\item (Empirische) Kritik Ende der 70er durch die Psychologen \person{Kehnemann} und \person{Tversky} ($\implies$ ``Prospect Theory'')
	\end{itemize}
\end{*remark}
\begin{proof}
	\begin{itemize}
...
\begin{definition}
	...
	\begin{enumerate}
		\item Für $X \in L_1$ heisst $c:= c_{\ast}(X,U) \in \R$ mit $c \sim_U X$ das \begriff{certainly equivalent} von $X$.
		\item Für $U \in C^2$ heissen
		\begin{align*}
			A_U(x) = - \frac{U''(x)}{U'(x)} \quad R_U(x) = -\frac{xU''(x)}{U'(x)}
		\end{align*}
		die \begriff{\person{Arrow}-\person{Pratt}-Koeffizienten} der absoluten bzw. relative Risikoaversion.
	\end{enumerate}
\end{definition}
\begin{*remark}
	\begin{itemize}
...
\begin{*example}[St. Petersburger Paradoxon (Nicholas Bernoulli, 1713)]
	\begin{itemize}
