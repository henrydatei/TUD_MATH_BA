...
\begin{definition}[Abbildung]
	...
	\begin{itemize}
		...
		\item $F(M'):=\{n\in M \mid n=F(m)$ für ein $m\in M'\}$ ist\begriff{Bild}\highlight{ von $M'$}$\subset M$
		...
		\item \mathsymbol{graph}{$\graph$}$(F) :=\{ (m,n)\in M\times N \mid n = F(m)\}$ heißt\begriff{Graph}\highlight{von $F$}
...
\begin{definition}[Isomorphismus]
	\begriff{Isomorphismus}bezüglich einer Struktur ist bijektive Abbildung $I:M_1\rightarrow M_2$, die auf $M_1$ und $M_2$ vorhandene Struktur erhält, z.B. ($i=1,2$)
	\begin{itemize}
		\item Ordnung "`$\le$"' auf $M_i$, falls: $a \le_1 b \;\Leftrightarrow \; I(a) \le_2 I(b)$ 
		\item Abbildung $F_i: M_i\to M_i$, falls: $I(F_1(a)) = F_2(I(a))$
		\item Rechenoperation $*_i: M_i \times M_i \to M_i$ falls $I(a *_1 b) = I(a) *_2 I(b)$
		\item spezielles Element $a_i\in M_i$, falls $I(a_1) = a_2$
	\end{itemize}
	
	Mengen $M_1$ und $M_2$ heißen\begriff[Menge!]{isomorph}, wenn es einen Isomorphismus gibt.
\end{definition}
...
\begin{definition}
$\mathbb{N}$ sei Menge, die die\begriff{\person{Peano}-Axiome}erfüllt, d.h.
\begin{enumerate}[label={P\arabic*)}]
	\item $\mathbb{N}$ sei induktiv, d.h. es ex.
	\begin{itemize}
	...
	$\Rightarrow N=\mathbb{N}$ ($N$ ist die kleinste induktive Menge)
\end{enumerate}
...
\begin{satz}[Rekursive Definition / Rekursion]
	\begriff*{Rekursion}
	Sei $B$ Menge, $b\in B$ u. $F:B\times\mathbb{N}\rightarrow B$ Abbildung. Dann liefert die Vorschrift \begin{align*}
...
\begin{conclusion}
	...
	\begin{enumerate}[label={\alph*)}]
		...
		\item $k\neq 0: m\cdot k = n\cdot k \Leftrightarrow m = n$ (Kürzungsregel Multiplikation)
	\end{enumerate}
\end{conclusion}
...
\begin{definition}
	Definiere Äquivalenzrelation $Q:=\{ ((n_1,n_1'),(n_2,n_2'))\in((\mathbb{N}\times\mathbb{N})\times(\mathbb{N}\times\mathbb{N})) \mid n_1+n_2' = n_1' + n_2 \}$
\end{definition}
\begin{satz}
...
\subsection*{Rechenoperationen}
\begin{definition}
	\begriff{Addition}[!ganze Zahlen]: $\overline{m}+\overline{n} = [(m,m')] + [(n,n')] :=[(m+n,m'+n')]$
...
\begin{definition}
	Betr. Relation $R:=\{(\overline{m},\overline{n})\in\overline{\mathbb{Z}}\times\overline{\mathbb{Z}} \mid \overline{m} \le \overline{n}\}$, wobei $\overline{m} = [(m,m')] \le [(n,n')]$ \gls{gdw} $(m+n'\le m'+n)$
\end{definition}
...
\begin{definition}
	Betr. $\mathbb{Z} = \mathbb{N}\cup\{ (-k) \mid k\in\mathbb{N}_{>0} \}$ mit üblicher Addition, Multiplikation und Ordnung "`$\ge$"'.
\end{definition}
\begin{satz}
...
\begin{definition}
	...
	Damit:
	\begin{itemize}
		\item $(-n)a := n(-a), 0_\mathbb{N} a := 0_K$ für $n\in\mathbb{N}_{\ge 1}$
		\item $ma + na = (m+n)a, na + nb = n(a+b)$
		\item $(ma)\cdot(na) = (mn)a^2, (-n)a = -(na)$
	\end{itemize}
	\item\begriff{Potenz}: $a^n$ von $a\in K, n\in\mathbb{Z}$ ist $a^n:=\prod_{k=1}^{n} a$
...
\begin{definition}[Intervall]
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item $[a,b]:=\{ x\in K \mid a \le x \le b \}$\begriff[Intervall!]{abgeschlossen}
			\item $(a,b):=\{x \in K \mid a < x < b\}$\begriff[Intervall!]{offen}
			\item $[a,b) := \{x \in K \mid a \le x < b\}, (a,b]:=\{x \in K \mid a < x \le b\}$\begriff[Intervall!]{halboffen}
		\end{itemize}
...
\begin{definition}[Folge]
    Eine\begriff{Folge}in Menge $M$ ist eine Abbildung $\alpha:\mathbb{N}\rightarrow M$ (evtl. $\alpha:\mathbb{N}_{\ge n}\rightarrow M$), $\alpha_n := \alpha(n)$ heißen\begriff{Folgenglieder}, und $n$\begriff{Folgenindex}.
...
\begin{definition}
	$Q:=\{ (\{X_n\}, \{Y_n\})\in I_\mathbb{Q}\times I_\mathbb{Q} \mid X_n \cap Y_n \neq \emptyset \,\forall n\in\mathbb{N} \}$ ist Relation auf $I_\mathbb{Q}$, $I_\mathbb{Q}:=$ Menge aller Intervallschachtelungen $\mathcal{X}=\{X_n\}$ in $\mathbb{Q}$ (d.h. $X_n\subset \mathbb{Q}$).
\end{definition}
...
\begin{definition}
	...
	\begin{itemize}
		...
		\item $-X := [-x,-x']$, $X^{-1}:=[\frac{1}{x'}, \frac{1}{x}]$ falls $0\notin X$
	\end{itemize}
...
\begin{definition}
	Betr. Relation "`$\le$"': $R:=\{ ([\{x_n\}],[\{y_n\}])\in\mathbb{R}\times\mathbb{R} \mid x_n \le y_n\,\forall n\in\mathbb{N}\}$
\end{definition}
\begin{satz}
...
\begin{definition}
	...
	\begin{itemize}
		...
		\mathsymbol{sup}{$\sup$}$ M:= \tilde{s} \le s$ (\mathsymbol{inf}{$\inf$}$ M = s \ge \tilde{s}) \;$ für obere (untere) Schranken $s\in M$.
	...
	Man hat
	\begin{alignat*}{4}
	\sup \,&M &= \min & \{s \mid s &\text{ obere }&\text{Schranke von } M\}\\
	\inf \,&M &= \max & \{s \mid s &\text{ untere }&\text{Schranke von } M\}
	\end{alignat*}
\end{definition}
\stepcounter{theorem}
\begin{satz}
...
\begin{theorem}
	...
	\[ K \text{ vollständig } \Leftrightarrow \sup M \slash \inf M \text{ ex. }\forall M\subset K, M\neq \emptyset \text{ nach oben \slash \, unten beschränkt} \]
\end{theorem}
...
\begin{definition}[Potenz]
	...
	Allgemein für $r\ge 0, a > 1: a^r := \sup \{ a^q \mid 0 \le q \le r,q\in\mathbb{Q} \}$
...
\begin{definition}[Logarithmus]
	Sei $a,b\in\mathbb{R}_{<0}, a\neq 1$:\begriff{Logarithmus}\highlight{von $b$ zur Basis $a$} ist \begin{align*}
	 \log_a b :=\begin{cases}
	 \sup \{ r \in \mathbb{R} \mid a^r \le b\}& \text{falls }a > 1\\
 	\sup \{r\in\mathbb{R}\mid a^r \ge b\}& \text{falls }0 < a < 1
	 \end{cases}
	\end{align*}
\end{definition}
...
\begin{example}
	...
	$\Rightarrow (Y,\tilde{d})$ ist metrischer Raum mit\begriff{induzierte Metrik}$\tilde{d}(x,y) := d(x,y)\,\forall x,y\in Y$.
\end{example}
...
\begin{satz}\label{satz_topologie}
	...
	\begin{enumerate}[label={\arabic*)}]
		...
		\item \label{topologie_3} $\bigcup_{U\in\tau'} U\in\tau$ falls $\tau'\subset\tau$ 
	\end{enumerate}
\end{satz}
\begin{conclusion}
...
\begin{definition}[konvergent]
	...
	Notation: $x=$\mathsymbol{lim}{$\lim\limits_{n\rightarrow\infty} x_n$}, $x_n\rightarrow x$ für $n\rightarrow\infty$, $x_n \overset{n\rightarrow\infty}{\longrightarrow}x$
...
\begin{satz}\label{tfprinzip}
	...
	\begin{enumerate}[label={\arabic*)}]
		\item $x_n\rightarrow x \;\Rightarrow\; x_{n_k} \overset{n\rightarrow\infty}{\longrightarrow} x$ für jede \gls{tf} $\{x_{n_k}\}$
		...
		\item\begriff{Teilfolgenprinzip}: Jede \gls{tf} $\{x_{n'}\}$ von $\{x_n\}$ hat \gls{tf} $\{x_{n''}\}$ mit $x_{n''}\rightarrow x$ $\Rightarrow$ $x_n \rightarrow x$
	\end{enumerate}
\end{satz}
\begin{satz}
...
\begin{satz}[Konvergenz in $\mathbb{R}^n$/$\mathbb{C}^n$ bzgl. Norm]
	Sei $\{x_n\}$ Folge mit $x_n = (x_n^1, \dotsc, x_n^n)\in\mathbb{R}^n (\mathbb{C}^n)$, $x=(x^1, \dotsc,x^n)\in\mathbb{R}^n (\mathbb{C}^n)$.
	
	$\lim\limits_{n\rightarrow\infty} x_n = x$ in $\mathbb{R}^n (\mathbb{C}^n)$ $\Leftrightarrow$ $\lim\limits_{n\rightarrow\infty} x_k^j = x^j$ in $\mathbb{R}$ bzw. $\mathbb{C}\,\forall j=1,\dotsc,n$
\end{satz}
...
\begin{definition}[monoton]
	...
	Falls stets "`$<$"' ("`$>$"') ist $\{x_n\}$\begriff[monoton!]{strikt}monoton.
\end{definition}
\begin{satz}
...
\begin{definition}
	...
	\mathsymbol*{limsup}{$\limsup$}\mathsymbol*{liminf}{$\liminf$}
	\begin{alignat*}{4}
		\limsup\limits_{n\rightarrow\infty} x_n &:= \overline{\lim}_{n\rightarrow\infty} x_n &=:& \sup H &\quad& \text{\begriff{Limes superior}von $\{x_n\}$} \\
		\liminf\limits_{n\rightarrow\infty} x_n &:= \underline{\lim}_{n\rightarrow\infty} x_n  &=:& \inf H  &\quad& \text{\begriff{Limes inferior}von $\{x_n\}$}
	\end{alignat*}
\end{definition}
...
\begin{definition}[Uneigentliche Konvergenz]
	...
	Notation: $\lim\limits_{n\rightarrow\infty} x_n = \pm \infty$ bzw. $x_n\rightarrow \pm \infty$
\end{definition}
\stepcounter{theorem}
...
\begin{definition}[Durchmesser]
	\begriff{Durchmesser}von $M\subset X$ beschränkt, $\neq \emptyset$, $(X,d)$ metrischer Raum ist \mathsymbol{diam}{$\diam$}$M:=\sup\{d(x,y) \mid x,y\in M\}$
...
\end{definition}
\begin{lemma}
	Sei $M\subset X$ beschränkt, $\neq \emptyset\;\Rightarrow\;\diam M = \diam (\cl M)$.
\end{lemma}
\begin{theorem}
...
\begin{lemma}
	...
	\begin{center}
		\begin{tabular}{lcl}
			$\bigcap\limits_{n\in\mathbb{N}} A_n\neq \emptyset$ & $\Leftrightarrow$ & $\bigcap\limits_{n\in\mathbb{N}} X_n\neq \emptyset$ \\[5pt]
...
\section{Kompaktheit}
\begin{definition}
Sei $(X,d)$ metrischer Raum, Mengensystem $\mathcal{U}\subset \{ U\subset X \mid U \text{ offen }\}$ heißt\begriff{offene Überdeckung}von $M\subset X$, falls $M\subset \bigcup_{U\in\mathcal{U}} U$.
...
Menge $M\subset X$ heißt\begriff{folgenkompakt}, falls jede Folge $\{x_n\}$ aus $M$ (d.h. $x_n\in M\,\forall n$) eine konvergente Teilfolge $\{x_{n'}\}$ mit Grenzwert in $M$ bessitzt (d.h. $\{x_n\}$ hat \gls{hw} in $M$ nach \ref{tfprinzip}).
\end{definition}
...
\begin{definition}[Partialsumme]
	...
	Existiert der Grenzwert $s = \lim\limits_{n\rightarrow\infty} s_n$, so heißt dieser\begriff[Reihe!]{Summe}der Reihe.\\
...
\begin{satz}[Konvergenzkriterien für Reihen]
	...
	\begin{enumerate}[label={\alph*)}]
		...
		\begin{enumerate}[label={\alph*)}]
			...
			\item $\frac{\Vert x_{k+1}\Vert}{\Vert x_k\Vert}\ge 1\,\forall k\ge k_0\;\Rightarrow \sum_k\Vert x_k\Vert$ divergiert.
		\end{enumerate}
...
\begin{satz}[Doppelreihensatz]
	Sei $\{x_{k,l}\}_{k,l\in\mathbb{N}}$ Doppelfolge im \person{Banach}-Raum $X$ und mögen $\sum_{l=0}^\infty \Vert x_{k,l}\Vert =:\alpha_k\,\forall k$ und $\sum_{k=0}^\infty \alpha_k =: \alpha$ existieren.
...
\section{Funktionen}
\begin{definition}
	$f:\mathbb{R}\to \mathbb{R}$\begriff{monoton}\begriff[monoton!]{fallend}/\begriff[monoton!]{wachsend}, falls $x < y, x,y\in M \,\Rightarrow \,f(x) \le f(y)$ bzw. $f(x) \ge f(y)$
...
\begin{satz}\label{Polynomdiv}
	...
	\begin{enumerate}[label={\arabic*)}]
		...
		$\Rightarrow$ $f(z) = g(z) \,\forall z\in\mathbb{C}$ (d.h. $a_k = b_k\,\forall k$)
	\end{enumerate}
\end{satz}
\stepcounter{theorem}
\begin{definition}
...
\begin{definition}
	Lineare Abbildung $f:X\to Y$ heißt\begriff{beschränkt}[!lineare Funktion], falls $f$ beschränkt auf $\overline{B_1(0)}$, d.h. \begin{align}
		\tag{1}\exists\text{ konstante }c > 0: \Vert f(x)\Vert \le c\,\forall x \text{ mit } \Vert x\Vert \le 1
	\end{align}
	Wegen $\left\lVert f\left( \frac{x}{\Vert x \Vert}\right) \right\rVert = \frac{1}{\Vert x \Vert} \Vert f(x) \Vert$ ist (1) äquivalent zu
	\begin{align}
		\tag{1'} \Vert f(x) \Vert = \sup \{ \Vert f(x) \Vert \mid x \in \overline{B_1(0)}\}
	\end{align}
\end{definition}
\begin{satz}
	Seien $X,Y$ normierte Räume über $K$, dann:\\
	\mathsymbol{L}{$L$}$(X,Y):= \{ f:X\to Y \mid f \text{ linear und beschränkt} \}$ ist normierter Raum über $K$ mit $\Vert f \Vert = \sup \{ \Vert f(x) \Vert \mid x\in \overline{B_1(0)} \}$
\end{satz}
...
\begin{satz}
	...
	\begin{enumerate}[label={\arabic*)}]
		...
		\item $\begin{alignedat}{2}
				\sin z - \sin w &= &2&\cos \frac{z+w}{2} \cdot \sin \frac{z+w}{2}\\
			  \cos z - \cos w &= -&2&\sin\frac{z+w}{2}\cdot\sin\frac{z-w}{2}\end{alignedat}$
	\end{enumerate}
\end{satz}
...
\begin{centering}
	\begin{tabular}{c|ccccc}
		...
		\midrule
		$\sin x$ & $0$ & $\frac{1}{2}$ & $\frac{\sqrt{2}}{2}$ & $\frac{\sqrt{3}}{2}$ & $1$ \\[5pt]
...
\begin{definition}
	$\tan z := \frac{\sin z}{\cos z}\,\forall z\in\mathbb{C}\setminus\{ \left. \frac{\pi}{2} + k\pi \middle| k\in\mathbb{Z}\right.\}$\\
	$\cot z := \frac{\cos z}{\sin z}\,\forall z\in\mathbb{C}\setminus \{ k\pi \mid k\in\mathbb{Z}\}$
...
\begin{satz}
	...
	\begin{enumerate}[label={\arabic*)}]
		...
		$w^n = z \,\Leftrightarrow\, w\in\left\{ \left. \sqrt[n]{z} e^{i \frac{k}{n} + \frac{2k\pi}{n}} =: w_k \,\middle|\, k=1,\dotsc,n\right.\right\}$ (Lösungen bilden ein regelmäßiges $N$-Eck auf dem Kreis mit dem Radius $\sqrt[n]{|z|}$)
	\end{enumerate}
\end{satz}

\subsection*{Logarithmen in $\mathbb{C}$} (sog. Hauptzweig)
\begin{definition}
	$\exp\left( \{ z\in\mathbb{C}\mid \Im z < \pi \}\right) \to \mathbb{C}\setminus (\infty, 0]$ ist bijektiv \\
...
\begin{definition}
	\begin{itemize}
		...
		\item $\coth(z) = \frac{\cosh(z)}{\sinh(z)} \,\forall z\in\mathbb{C}\setminus \{ k\pi \mid k\in\mathbb{Z}\}$ (\begriff{Cotangens Hyperbolicus})
	\end{itemize}
\end{definition}
...
\begin{satz}
	...
	\begin{center}
		$f_n \to f$ gleichmäßig auf $X$ $\Leftrightarrow$ $f_n \to f$ in $(B(X,Y),\Vert.\Vert_\infty)$
	\end{center}
\end{satz}
...
\rule{4cm}{0.4pt}
\begin{definition}
	Sei $f:D\subset X\to Y$ metrischer Raum, $D=\mathcal{D}(f)$, Fkt. $f$ heißt\begriff{folgenstetig}im Punkt $x_0\in D$, falls \[ f(x_n)\to f(x_0) \quad\forall \text{ Folgen $x_n\to x_0$ in $D$} \]
\end{definition}
\stepcounter{theorem}
\begin{definition}
	...
	\begin{tabularx}{\textwidth}{lX}
		\noindent\highlight{Interpretation:} & An Input / Output Steuerung besteht Forderung, dass beliebig kleine Output-Toleranzen $\epsilon$ stets durch hinreichend kleine Input-Toleranzen $\delta$ erreicht werden können.
	\end{tabularx}
\end{definition}
...
\begin{example}[\person{Dirichlet}-Funktion]
	...
	\end{cases} \] ist in keinem $x_0\in\mathbb{R}$ stetig.
\end{example}
\begin{satz}
...
\begin{example}
	...
	$X=\mathbb{R}^{n+1}, X_0 := \{(x_0, \dotsc, x_{n+1}) \in\mathbb{R}^{n+1} \mid x_{n+1}=0\}, N = (0,\dotsc, 0,1)$ (Nordpol), $S_n = \{ x\in\mathbb{R}^{n+1} \mid \vert x\vert=1\}$ $n$-dimensionale Einheitsspäre.
	
	Betrachte $\sigma: \mathbb{R}^{n+1} \setminus\{ N\} \rightarrow \mathbb{R}^{n+1}$ mit $\sigma(x) = N \frac{2}{(x-N)^2}\langle N, x-N\rangle$ stetig. $\sigma$ ist Homöomorphismus mit $\sigma^{-1}(y) = N - \frac{2}{(y-N)^2}\langle N,Y-N\rangle$
\end{example}
\rule{4cm}{0.4pt}
\begin{satz}
...
\begin{definition}[\index{Pol der Ordnung $k$}Pol der Ordnung $k$]
    ...
    \[
    R(z) = \frac{\tilde{f}(z)}{(z-z_0)^k\cdot \tilde{g}(z)} \text{ und } \tilde{f}(z_0) \neq 0, \tilde{g}(z_0) \neq 0.
    \]
