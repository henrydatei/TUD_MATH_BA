...
\begin{example}
	\begin{enumerate}[label=(\alph*)]
		\item $\C$ ist algebraisch abgeschlossen (Fundamentalsatz der Algebra, \propref{2_4_4})
...
\begin{proof}
	...
	\begin{flalign*}
		\qquad &\Halb := \big\lbrace(M,\sigma) \;\big|\; K \subseteq M \subseteq L \text{ Zwischenkörper, }\sigma\in \Hom(M,E),\, \sigma|_{K} = \varphi\big\rbrace &\\
		\intertext{mit der Ordnung}
		&(M,\sigma) \subseteq (M' , \sigma') :\mskip-4mu\iff M \subset M' \und \sigma'|_{M} = \sigma&
	\end{flalign*}
	Dann ist
	\begin{itemize}
		\item $\Halb \neq \emptyset$: $(K,\varphi) \in \Halb$
		\item Ist $(M,\sigma)_{i \in I}$ eine Kette in $\Halb$, so definieren wir $M:= \bigcup_{i\in I} M_i$ und $\sigma\colon M \to E$ durch $\sigma(x) = \sigma_i (x)$ falls $x \in M_i$. Dann ist $(M,\sigma) \in \Halb$ eine obere Schranke der Kette $(M_i , \sigma_i)_{i\in I}$.
\end{itemize}	
	
	Nach dem Lemma von \person{Zorn} existiert $(M, \sigma)$ maximal. 
...
\begin{proof}\leavevmode\vspace*{\dimexpr-\baselineskip+2\lineskip}
	\begin{itemize}
		...
		\begin{proof}
			...
			\begin{flalign*}
				\qquad &\varphi|_{K} = \id_K, \quad \varphi(X_{f_i}) = \alpha_i, \quad \varphi(X_f) = 0 \text{ für } f \in \lnkset{\mathscr{F}}{\set{f_1,\dots, f_n}}&
			\end{flalign*}
			Dann folgt
			\begingroup
			\zeroAmsmathAlignVSpaces*
			\begin{flalign*}
				...
				= \sum_{i=1}^n \varphi(g_i) \cdot f_i (\underbrace{\varphi(X_{f_i})}_{= \alpha_i}) = \sum_{i=1}^n \varphi(g_i) \cdot \underbrace{f_i (\alpha_i)}_{=0} = 0 &
			\end{flalign*}
			\endgroup
		\end{proof}
...
\begin{definition}[konjugiert]
	$\alpha, \beta \in \bar{K}$ sind $K$-\begriff{konjugiert} $\Longleftrightarrow$ existiert $\sigma \in \Aut(\bar{K}\mid K)\mit \sigma(\alpha) = \beta$.
\end{definition}
\begin{remark}
