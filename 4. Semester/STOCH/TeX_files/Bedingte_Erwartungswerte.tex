...
\begin{proof}\
	\begin{enumerate}
		...
		\]
		dann ist $\mu \ll\P_Y$. Aus dem Satz von \person{Randon}-\person{Nikodym} (\propref{6_1_1:Radon}) folgt die Existenz einer $\P_Y$-f.s. bestimmten Dichte $g$.
		...
		\end{align*}
		Sind $g_1,g_2$ zwei Versionen der bedingten Erwartung, dann gilt wegen $\E[X^-]<\infty$ auch $\int_{\O_Y} g_1^- (y)\P_Y(\d y) < \infty \quad i=1,2$
		\begin{align*}
		\implies \int_B g^+_1(y)\P_Y(\d y) = \int_B g_1^-(y)\P_Y(\d y)\\
		&= \int_B g_1(y) \P_Y(\d y)\\
		&= \int_B g_2 (y)\P_Y (\d y)\\
		...
		&\implies g_1^+ - g_1^- = g_1 = g_2 = g_2^+ - g_2^- \P_Y \text{-f.s.}
		\end{align*}
	\end{enumerate}
\end{proof}
\begin{*remark}
