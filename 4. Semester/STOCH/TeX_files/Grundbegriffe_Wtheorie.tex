...
und insbesondere $\probp(A) + \probp(A^C) = 1$
		...
		\begin{align*}
			\probp(A_n) \xrightarrow[n \to \infty]{} \P(A)
		\end{align*}
	\end{enumerate}
\end{proposition}
...
\begin{example}
	...
	\begin{align}
		...
		\end{cases}\notag
	\end{align}
	ein (degeneriertes) W-Maß auf $(\Omega, \sigF)$, welches wir als \begriff{\person{Dirac}-Maß} oder \begriff{\person{Dirac}-Verteilung} bezeichnen.
\end{example}
...
\begin{example}
	\proplbl{1_7}
	Wartezeit an der Bushaltestelle mit Ergebnisraum $\Omega = \real_{+}$ und Ereignisraum \person{Borel}sche $\sigma$-Algebra $\borel(\real_{+}) = \sigF$. Ein mögliches W-Maß können wir dann durch
	\begin{align}
...
\begin{proposition}[Konstruktion von Wahrscheinlichkeitsmaßen durch Dichten]
	...
	\begin{itemize}
		...
		\end{align*}
		ein (diskretes) Wahrscheinlichkeitsmaß $\probp$ auf $(\O, \F)$. $\rho$ wird als \begriff{Zähldichte} bezeichnet.
		...
		$\rho$ bezeichnen wir als \begriff{Dichte}, \begriff{Dichtefunktion}/\begriff{Wahrscheinlichkeitsdichte} von $\probp$ und nennen ein solches $\probp$ \begriff{(absolut) stetig (bzgl. denn Lebesgue-Maß)}.
	\end{itemize}
\end{proposition}
...
\end{align}
bereits die Exponentialverteilung aus \propref{1_7}.
...
\begin{definition}[Wahrscheinlichkeitsraum]
	Ein Tripel $(\O, \F, \P)$ mit $\O, \F$ Ereignisraum und $\probp$ Wahrscheinlichkeitsmaß auf $(\O, \F)$, nennen wir \begriff{Wahrscheinlichkeitsraum}.
\end{definition}
...
\begin{definition}[Zufallsvariable]
	...
	\begin{align*}
		X: \Omega \to \Omega'
	\end{align*}
	\begriff{Zufallsvariable} (von $(\O, \F)$) nach $(\O', \F')$/ auf $(\O, \F)$ oder \begriff{Zufallselement}.
\end{definition}
...
\begin{proof}
	Folgt aus \propref{1_8}, der \cref{2_5} der Verteilungsfunktion und dem Eindeutigkeitssatz \propref{1_9}.
\end{proof}