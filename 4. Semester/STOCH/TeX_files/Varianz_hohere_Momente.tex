...
\begin{proposition}[Eigenschaften der (Ko-)varianz]
	...
	\begin{enumerate}
		...
		$\abs{\Corr(X,Y)} \le 1$
		\item $\Var\brackets{\sum_{i=1}^n X_i} = \sum_{i=1}^n \Var(X_i) + \sum_{i=1}^n \Var(X_i,X_j)$ Sind die $X_1, \dots, X_n$ paarweise unkorreliert, so gilt die \begriff{Formel von \person{Bienaymé}}:
		\[
