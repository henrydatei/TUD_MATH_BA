...
\end{proposition}
\begin{proof}
	Aus $\Vert f(x)\Vert=\Vert x\Vert$ folgt $\skalar{f(x)}{f(x)}=\skalar{x}{x}$. Die Polarisierung (\propref{2_3_4}) für $\skalar{f(x)}{f(y)}$ und die Linearität von $f$ liefern $\skalar{f(x)}{f(y)}=\skalar{x}{y}$. Zum Beispiel im Fall $K=\real$:
	\begin{align}
...
\end{proposition}
\begin{proof}
	Sei $s$ das Standardskalarprodukt und $B=(a_1,...,a_n)$. Nach \propref{2_4_5} ist $B$ genau dann eine Orthonormalbasis, wenn $M_B(s)=\mathbbm{1}_n$, und $M_B(s)=A^t\cdot\mathbbm{1}_n\cdot\overline{A}$, vgl. \propref{2_2_9}
\end{proof}
...
\begin{proof}
	...
	\emph{$n=0$}: klar \\
	\emph{$n-1\to n$}: Da $K$ algebraisch abgeschlossen ist, hat $\chi_f$ eine Nullstelle $\lambda$, es gibt also einen Eigenvektor $x_1$ von $f$ zum Eigenwert $\lambda$. Ohne Einschränkung nehmen wir $\Vert x\Vert=1$ an. Sei $W=K\cdot x_1$. Nach \propref{2_4_11} ist dann $V=W\oplus W^\perp$. Für $v\in W^\perp, w\in W$ ist
	\begin{align}
...
\end{conclusion}
\begin{proof}
	Da $A$ unitär ist, ist $f_A\in\End_\comp(\comp^n)$ unitär, nach \propref{2_5_9} existiert also eine Orthonormalbasis $B$ des $\comp^n$ aus Eigenvektoren von $A$. Die Transformationsmatrix $S=T_{\mathcal{E}}^B$ hat als Spalten die Elemente von $B$ und somit ist $S$ nach \propref{2_5_8} unitär. Nach \propref{2_5_2} ist $\vert\lambda\vert=1$ für alle Eigenwerte von $f_A$.
\end{proof}

\begin{remark}
	Dies (\propref{2_5_9}) gilt nicht im Fall $K=\real$. Man kann aber auch orthogonale Endomorphismen immer "'fast diagonalisieren"'.
\end{remark}