...
\begin{conclusion}
	...
	Sei $M\subset\mathbb{R}^n$ messbar. Dann gilt:\begin{enumerate}[label={\alph*)}]
		...
		\begin{tabularx}{\linewidth}{X@{\ \ }c@{\ \ }X}
			\hfill $\displaystyle \int_M \vert h \vert \D x = 0$ & $\Leftrightarrow$ & $h=0$ f.ü. auf $M$
		\end{tabularx}
	\end{enumerate}
...
\end{align}
nach \propref{messbarkeit_funktion_approximation} sollte man in \eqref{integral_messbare_funktion_forderung} eine Folge von Treppenfunktionen $\{ h_k\}$ mit $h_k(x)\to f(x)$ f.ü. auf $M$ betrachten, \emph{aber} es gibt zu viele konvergente Folgen für einen konsistenten Integralbegriff.
...
\begin{example}
	...
	Betrachte $f=0$ auf $\mathbb{R}$, wähle beliebige Folge $\{\alpha_k\}\subset\mathbb{R}$, dazu eine Treppenfunktion \begin{align*}
		...
		\end{cases}
	\end{align*}
	Offenbar konvergiert $h_k$ gegen $0$ f.ü. auf $\mathbb{R}$ und man hat $h_k\to 0$ f.ü. auf $\mathbb{R}$ und $\int_{\mathbb{R}} h_k \D x = \alpha_k$
...
\begin{*definition}[$L^1$-\person{Chauchy}-Folge, \person{Lebesgue}-Integral]
	...
	Messbare Funktion $f:D\subset\mathbb{R}^n\to\overline{\mathbb{R}}$ heißt \begriff{integrierbar} auf $M\subset D$, falls Folge von Treppenfunktionen $\{ h_k\}$ in $T^1(M)$ existiert mit $\{ h_k\}$ ist $L1$-CF auf $M$ und $H_k\to f$ f.ü. auf $M$.\marginnote{\leqnos\begin{align}\proplbl{integral_funktion_definition}\,\end{align}Formel (3) unbekannt}[-1.5\baselineskip]
...
\begin{remark}\vspace*{0pt}
	\begin{enumerate}[label={\alph*)},topsep=\dimexpr -\baselineskip / 2\relax]
		...
		\item Sei $0\le h_1 \le h_2 \le \dotsc$ monotone Folge von integrierbaren Treppenfunktionen mit $h_k\to f$ f.ü. auf $M$ und sei Folge $\{ \int_M h_k\D x\}$ in $\mathbb{R}$ beschränkt \\
...
\begin{proposition}[Eigenschaften]
	...
	Es gilt \begin{enumerate}[label={\alph*)}]
		...
		Seien $f$, $g$ integrierbar auf $M$. Dann \begin{center}
			$f\le g$ f.ü. auf $M$ \ \ $\Rightarrow$ \ \ $\displaystyle\int_M f\D x \le \int_M g \D x$
		\end{center}
		\item\proplbl{integral_funktion_eigenschaften_nullfunktion}
		 Sei $f$ integrierbar auf $M$, dann \begin{center}
				$\displaystyle \int_M \vert f \vert \D x = 0$\ \ $\Leftrightarrow$ \ \ $f = 0$ f.ü.
		\end{center}
	\end{enumerate}
...
\begin{proof}\hspace*{0pt}
	...
	\begin{enumerate}[label={zu \alph*)},topsep=\dimexpr -\baselineskip / 2\relax,leftmargin=\widthof{\texttt{zu b)\ }}]
		...
		\ $\Rightarrow$ $\vert h_k \vert\to \vert f \vert$ f.ü. auf $M$.
		...
		\item Nach den Rechenregeln ist $g - f$ integrierbar, wegen $\vert g - f\vert = g - f$ f.ü. auf $M$ folgt \begin{align*}
		...
		\item[zu a)] für "`$\Leftarrow$"' wähle $f^\pm$ ($ f = f^+ - f^-$) jeweils eine monotone Folge von \gls{tf} $\{ h_k^\pm \}$ gemäß \propref{messbarkeit_funktion_existenz_monotone_treppenfunktionen}. Folglich liefert $H_k = h_k^+ - h_k^-$ eine Folge von \gls{tf} mit $h_k\to f$ f.ü. auf $M$.
		
		Wegen $\vert h_k \vert \le \vert f \vert$ f.ü. auf $M$ ist $\int_M \vert h_k\vert \D x \le \int_M \vert f \vert \D x$.
		...
		\item Für $f=0$ f.ü. auf $M$ ist offenbar $\int_M \vert f\vert \D x = 0$.
...
\begin{conclusion}
	...
	Sei $f$ auf $M$ integrierbar\begin{enumerate}[label={\alph*)}]
		\item Für $\alpha_1$, $\alpha_2\in\mathbb{R}$ gilt:\begin{center}
			$\alpha_1\le f \le \alpha_2$ f.ü. auf $M$ \ \ $\Rightarrow$ \ \ $\displaystyle \alpha_1 \vert M \vert \le \int_M f \D x \le \alpha_2 \vert M \vert$
		\end{center}
		\item Es gilt $f\ge 0$ f.ü. auf $M$ \ \ $\Rightarrow$ \ \ $\int_M f \D x\ge 0$
		\item Es gilt: $\tilde{M}\subset M$ messbar, $f\ge 0$ f.ü. auf $M$ \\
...
\begin{proof}\hspace*{0pt}
	...
	\begin{enumerate}[label={zu \alph*)},topsep=\dimexpr-\baselineskip/2\relax,leftmargin=\widthof{\texttt{zu a)\ }}]
		...
		\item folgt, da $\chi_{\tilde{M}}\cdot f \le f$ f.ü. auf $M$ und aus der Monotonie\hfill\csname\InTheoType Symbol\endcsname
	\end{enumerate}
\end{proof}
...
\begin{proposition}
	...
	\end{align*}
	Weiterhin gibt es eine Teilfolge $\{ f_{k'}\}$ mit $f_{k'}\to f$ f.ü. auf $M$.
\end{proposition}
...
\begin{proof}
	...
	$\Rightarrow$ & $f_{k_l} \xrightarrow{l\to\infty} f$ f.ü. auf $M$ \hfill\csname\InTheoType Symbol\endcsname
	\end{tabularx}
\end{proof}

\begin{proposition}[Majorantenkriterium]
	\proplbl{integral_funktion_majorantenkriterium}
	Seien $f$, $g:D\subset\mathbb{R}^n\to\overline{\mathbb{R}}$ messbar, $M$ messbar, $\vert f \vert \le g$ f.ü. auf $M$, $g$ integrierbar auf $M$ \\
...
\begin{lemma}
	...
	\end{align}
	$\Rightarrow$ $\{ h_k\}$ ist $L^1$-CF zu $f$ und falls $\{ h_k\}\to f$ f.ü. auf $M$ ist $f$ integrierbar (vgl \propref{messbarkeit_funktion_existenz_monotone_treppenfunktionen})
\end{lemma}
...
\begin{proof}
	...
	Falls noch $h_k\to f$ f.ü. $\Rightarrow$ $\{h_k\}$ ist $L^1$-CF zu $f$ $\Rightarrow$ $f$ ist integrierbar
\end{proof}
...
\begin{proof}[\propref{integral_funktion_majorantenkriterium}]
	...
	\end{align*}
	auf $M$ und $\{ h_k\}\to\vert f \vert$ f.ü. auf $M$.
...
\begin{proof}
	...
	Für jedes $k\in\mathbb{N}$ wählen wir gemäß \propref{messbarkeit_funktion_existenz_monotone_treppenfunktionen} eine Folge $\{ h_{k_l} \}_l$ von Treppenfunktionen mit $0\le h_{k_1} \le h_{k_2} \le \dotsc \le g_k$, $h_{k_l}\xrightarrow{l\to\infty} g_k$ f.ü. auf $M$.
	...
	Folglich $\tilde{h}_l\to f$ f.ü. auf $M$ und wegen der Monotonie ist $\{ \tilde{h}_k\}$ $L^1$-CF zu $f$ \\
...
\begin{theorem}[Monotone Konvergenz]
	...
	Seien $f_k:D\subset\mathbb{R}^n\to\overline{\mathbb{R}}$ integrierbar auf $M\subset D$ $\forall k\in\mathbb{N}$ mit $f_1 \le f_2 \le \dotsc $ f.ü. auf $M$ \\
...
\begin{remark}
	\propref{integral_grenzwertsatz_monotone_konvergenz} bleibt richtig, falls man $f_1 \ge f_2 \ge \dotsc$ f.ü. auf $M$ hat.
...
\begin{theorem}[Majorisierte Konvergenz]
	...
	Seien $f_k$, $g:D\subset\mathbb{R}^n\to\overline{\mathbb{R}}$ messbar für $k\in\mathbb{N}$ und sei $g$ integrierbar auf $M\subset D$ mit $\vert f_k\vert \le g$ f.ü. auf $M$ $\forall k\in\mathbb{N}$ und $f_k\to:f$ f.ü. auf $M$
	\begin{align}
...
\begin{proof}
	Nach dem Majorantenkriterium sind alle $f_k$ f.ü. integrierbar auf $M$.
