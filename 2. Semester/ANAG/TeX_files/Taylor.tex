...
\textbf{Vorbetrachtung:} Sei $X$ endlich dimensionaler, normierter Raum über $K$ (d.. Vektorraum über $K$ mit Norm $\Vert \,\cdot \Vert$, $\dim X =l\in\mathbb{N}$).
	...
	Betrachte nun $f:D\subset K^n\to K^m$, $D$ offen, $f$ \gls{differenzierbar} auf $D$. Falls $g:= f':D\to L(K^n, K^m) =: y_1$ \gls{differenzierbar} in $x\in D$ ist, heißt \begin{align}
...
\textbf{Frage:} höhere Ableitungen, d.h. von $f'':D\to L(K^n, Y_1)$ usw.
	...
	Endlich dimenionale, normierte Räume, man kann rekursiv $\forall k\in \mathbb{N}$ definieren: \begin{enumerate}[label={(\roman*)}]
		...
		\end{tabularx}
		Falls $f^{(k)}:D\to Y_k$ \gls{differenzierbar} in $x\in D$ heißt \[ f^{(k+1)}(x) := \left( f^{(k)} \right) (x)\in L(K^n, Y_k)  \]
...
\begin{*definition}[$k$-fach differenzierbar]
	...
	$f$ heißt $k$-fach stetig \gls{differenzierbar} (auf $D$) oder $C^k$-Funktion, falls $f$ $k$-fach \gls{differenzierbar} und $f^{(k)}:D\to Y_k$ stetig.
	
	$C^k(D, K^m) := \{ f:D\to K^m\mid $f$ \text{ $k$-fach stetig \gls{differenzierbar} auf $D$} \}$
\end{*definition}
...
\begin{theorem}
	...
	Sei $f:D\subset K^n\to K^m$, $D$ offen, $x\in D$. Dann \begin{enumerate}[label={(\alph*)}]
		...
		\ \ $\Rightarrow$ $f$ ist $k$-fach \gls{differenzierbar}, d.h. $f^{(k)}(x)$ existiert.
	\end{enumerate}
\end{theorem}

\begin{remark}
	\propref{taylor_partielle_ableitung_zusammenhang_hoehere_ableitung} \ref{taylor_partielle_ableitung_zusammenhang_hoehere_ableitung_b} ist ein wichtiges Kriterium zur Prüfung der \gls{differenzierbar}keit, $k$-te Ableitung kann dann mittels \eqref{taylor_partielle_ableitung_zusammenhang_hoehere_ableitung_eq} bestimmt werden.
\end{remark}
...
\begin{conclusion}
	...
	Sei $f:D\subset\mathbb{R}^n\to\mathbb{R}^m$, $D$ offen, $f$ $k$-fach \gls{differenzierbar} (d.h. $f\in C^k(D,\mathbb{R}^m)$) \\
...
\begin{proof}[\propref{taylor_partielle_ableitung_schwarz}]
	...
	Definiere $\phi(s) := f(x + s\cdot e_i + t\cdot e_j) - f(x + s\cdot e_i)$ ist \gls{differenzierbar} auf $(-\delta,\delta)$ $\forall t\in (-\delta,\delta)$ \\
...
\begin{proposition}[notwendige Integrabilitätsbedingung]
	...
	Sei $f=(f_1, \dotsc, f_n): D\subset\mathbb{R}^n\to\mathbb{R}^n$, $D$ Gebiet\marginnote{Gebiet: offen, zusammenhängend}, $f$ stetig \gls{differenzierbar}.
...
\begin{proposition}
	Sei $f:D\subset\mathbb{R}^n\to\mathbb{R}$, $D$ offen und konvex, $f$ stetig \gls{differenzierbar}. Dann:\begin{enumerate}[label={\alph*)}]
...
\begin{theorem}[\person{Taylor}-scher Satz]
	...
	Sei $f:D\subset K^n\to K^m$, $D$ offen, $k$-fach \gls{differenzierbar} auf $D$, $x\in D$. Dann \begin{align}
...
\begin{proof}
	...
	Da $f$ $k$-fach \gls{differenzierbar}\\
	\begin{tabularx}{\linewidth}{r@{\ \ }X}
	$\Rightarrow$ & $\phi:[0,1]\to K^m$ $\mathbb{R}$-\gls{differenzierbar} auf $(0,1)$ mit
	...
	\begin{enumerate}[label={(\alph*)}]
		...
		\begin{tabularx}{\linewidth}{r@{\ \ }X}
		$\Rightarrow$ & $\psi:[0,1]\to\mathbb{R}$ \gls{differenzierbar} auf $(0,1)$ mit $\psi'(t) = \langle \phi'(t),r\rangle$ \\
...
\begin{conclusion}[\person{Taylor}-Formel mit partiellen Ableitungen]
	Sei $f:D\subset K^n\to K^m$, $d$ offen, $f$ $k$-fach \gls{differenzierbar} auf $D$, $x\in D$, $[c,c+y]\subset D$: \begin{align}
