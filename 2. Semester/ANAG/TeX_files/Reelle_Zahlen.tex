...
\begin{proof}
	\begin{itemize}
		\item[1)] vgl. lineare Algebra (LA)
		\item[2)] betrachte $0 + 0 = 0$ bzw. $1 \cdot 1 = 1$
		\item[3)] $(-a) + a = 0 \overset{\text{kommutativ}}{\Rightarrow} a = -(-a)$, Rest analog
		\item[4)] $a + b= ((-a) + (-b)) \Rightarrow $ Behauptung Addition und Multiplikation analog
		\item[5)] $a\cdot 0 = 0$ vgl. LA\\ 
		$1a + (-1)a = (1-1)a = 0 \Rightarrow (-1)a = -a$\\
		$(-a)(-b) = (-1)(-a)b \overset{\text{3), 5)}}{\Rightarrow} ab$
		\item[6)] ``$\Leftarrow$'': nach 5)\\
		``$\Rightarrow$'': Sei $a \neq 0$ (sonst klar) $\Rightarrow 0 = a^{-1}\cdot 0 \overset{ab=0}{=} a^{-1}ab = b \Rightarrow$ Behauptung
		\item[7)]$a+x = b \Leftrightarrow  x = (-a)+a + x = (-a)+b$, für $ax=b$ analog
	\end{itemize}
\end{proof}
...
\begin{proof}
	hier sollte bald was stehen! \smiley
\end{proof}
