...
Betrachte $f:D\subset\mathbb{R}^n\to\mathbb{R}$\marginnote{Zielraum $\mathbb{R}$ wegen Ordnung}, $D$ offen, $f$ \gls{differenzierbar}.

\textbf{notwendige Bedingung:} (\propref{mittelwertsatz_optimalitaetsbedingung}): $f$ hat lokales Minimum / Maximum in $x\in D$ $\Rightarrow$ $f'(x) = 0$
...
\textbf{Test Definitheit in Anwendungen}$k=2$ wichtig (vgl. lineare Algebra). \\
...
\textbf{Motivation:} Für $m\ge 1$: notwendige Bedingung: $f'(\mathrm{max})$ steht senkrecht auf der Niveaumenge $G$ \marginnote{(vgl. \propref{richtungsableitung_gradient_eigenschaften})} \\
...
\textbf{Existenz:} nach \propref{chap_15_3}:
