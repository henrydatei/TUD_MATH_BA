...
\begin{hint}
	...
	\begin{tabularx}{\linewidth}{XcX}
		\hfill$f$ hat Minimum auf $D$ & $\xLeftrightarrow{\text{vgl. \cref{chap_5}}}$ & $\min\{ f(x) \mid x\in D \}$ existiert (d.h. $\inf \{\dotsc\}$ wird angenommen)
	\end{tabularx}
	Analog für Maximum.
\end{hint}
...
\begin{proof}
	...
	$\xRightarrow{\propref{chap_15_3}}$ $\exists x_1, x_2\in [a,b]: f(x_1) \le f(x) \le f(x_2)$ $\forall x$
	\begin{itemize}
		\item Angenommen, $f(x_1) = f(x_2) = f(a)$ $\Rightarrow$ $f$ konstante Funktion $\Rightarrow$ $f'(\xi) = 0$ $\forall \xi \in (a,b)$
		\item Andernfalls sei $f(x_1) < f(a)$ $\Rightarrow$ $\xi := x_1\in(a,b)$ $\xRightarrow{\propref{mittelwertsatz_optimalitaetsbedingung}}$ $f'(\xi) = 0$
...
\begin{proof}
	...
	\begin{tabularx}{\linewidth}{rX}
	\parbox{\widthof{$\xRightarrow{\propref{mittelwertsatz_rolle}}$}}{\hfill$\xRightarrow{\text{$f$ \gls{diffbar}}}$}& $\phi: [0,1]\to \mathbb{R}$ stetig, $\phi(0) = \phi(1) = f(x)$
	\end{tabularx}
	...
	\end{align}
	\begin{tabularx}{\linewidth}{rX@{}}
	\parbox{\widthof{$\xRightarrow{\propref{mittelwertsatz_rolle}}$}}{$\xRightarrow{\propref{mittelwertsatz_rolle}}$} & $\exists \tau \in (0,1): \phi'(t) = 0$ \\
...
\begin{proof}
	...
	\begin{tabularx}{\linewidth}{rX@{}}
		...
		$\xRightarrow{\propref{mittelwertsatz_rolle}}$ & $\exists \xi \in(x,y): 0 = h'(\xi) = \big( f(y) - f(x) \big) g'(\xi) - \big( g(y) - g(x) \big) f'(\xi)$ \\
		$\Rightarrow$ & Behauptung\hfill\csname\InTheoType Symbol\endcsname
	\end{tabularx}
\end{proof}
...
\begin{proof}
	...
	$\xRightarrow{\propref{chap_15_3}}$ $\Vert f'(\xi)\Vert$ besitzt Maxium auf $M$ und die Behauptung folgt aus \propref{mittelwertsatz_schrankensatz}.
\end{proof}
...
\begin{proof}
	...
	\begin{enumerate}[label={\arabic*.},topsep=-\baselineskip]
		\item 
	\begin{itemize}
	\item $D$ offen, zusammenhängend, $K^n$ normierter Raum  $\xRightarrow{\propref{satz_15_8}}$ $D$ bogenzusammenhängend
	...
	\begin{tabularx}{\linewidth}{rX}
		$\xRightarrow{\propref{mittelwertsatz_schrankensatz}}$ & $\vert f(y) - f(x) \vert \le \underbrace{\Vert f'(\xi) \Vert}_{= 0} \cdot \vert y - x\vert = 0$ \\
		$\Rightarrow$ & $f(x) = f(y)$ \\
...
\begin{example}
	...
	\begin{tabularx}{\linewidth}{rX}
	$\xRightarrow{\footnotesize\propref{mittelwertsatz_ableitung_null_konstante_funktion}}$ & $f(x) = \mathrm{const}$ auf $(0,1)$ und $(2,3)$, aber auf jedem Intervall kann die Konstante anders sein.
	\end{tabularx}
\end{example}
...
\begin{proof}
	...
	Offenbar $\phi_k(t) = f(a_{k-1} + t e_k y_k) - f(a_{k - 1}) - tf_{x_k}(a_{k - 1}) y_k$ stetig auf $[0,1]$, \gls{diffbar} auf $(0,1)$ mit $\phi_k'(t) = f_{x_k}(a_{k - 1} + t e_k y_k) y_k - f_{x_k}(a_{k-1}) y_k$ \\
	$\xRightarrow{\footnotesize \propref{mittelwertsatz_schrankensatz}}$ $\vert \phi_k(1) - \phi_k(0)\vert = \vert f(a_k) - f(a_{k  - 1}) - f_{x_k} (a_{k  +1}) y_k \vert \le \sup\limits_{t\in (0,1)} \vert \phi_k'(\xi)\vert$, $k = 1,\dotsc,n$
	...
	\begin{tabularx}{\linewidth}{r@{$\;$}c@{$\;$}c@{}l}
		...
		& $\overset{\footnotesize\triangle\text{-Ungl}}{\le}$& & $\displaystyle\sum_{k=1}^n \big\vert f(a_k) - f(a_{k - 1}) - f_{x_k}(x) y_k \big\vert$ \\
		& $\underset{\text{Def. $\phi_k$}}{\overset{\footnotesize\triangle\text{-Ungl}}{\le}}$& & $\displaystyle\sum \vert \phi_k(1) - \phi_k(0)\vert + \vert f_{x_k} (a_{k - 1}) y_k - f_{x_k}(x) y_k \vert$ \\
		...
		& $\overset{\footnotesize\triangle\text{-Ungl}}{\le}$ & $\vert y \vert$ & $\displaystyle \underbrace{\sum_{k=1}^n \sup \vert f_{x_k} (a_{k-1} + t e_k y_k ) - f_{x_k}(x) \vert + 2 \vert f_{x_k}\ (a_{k - 1}) - f_{x_k}(x) \vert} _{=:\rho(y) \xrightarrow{y\to 0}0\text{, da part. Ableitung $f_{x_k}$ stetig in $x$}}$
	\end{tabularx}
...
\begin{proof}
	...
	\end{tabularx}
	$\xRightarrow{\propref{mittelwertsatz_optimalitaetsbedingung}}$ $0 = g'(\tilde{x}) = f'(\tilde{x}) - \gamma$ $\Rightarrow$ Behauptung 
\end{proof}
...
\begin{proof}\hspace*{0pt}
	...
	\begin{enumerate}[topsep=\dimexpr-\baselineskip/2\relax,label={zu \roman*)},leftmargin=\widthof{\texttt{zu ii)}}]
		...
		$\xRightarrow{\propref{mittelwertsatz_mittelwertsatz_verallgemeinert}}$ $\forall x\in(a,b)\;\exists\xi = \xi(x) \in (a,x): \frac{f(x)}{g(x)} = \frac{f'(\xi)}{g'(\xi)}$. Wegen $\xi(x)\to a$ für $x\to a$ folgt die Behauptung
		...
		\begin{align*}
			\left\vert \frac{f(y) - f(x)}{g(y) - g(x)} - \gamma \right\vert \underset{\exists \xi\in(a,a+\delta)}{\overset{\footnotesize\propref{mittelwertsatz_mittelwertsatz_verallgemeinert}}{\le}} \underbrace{\left\vert \frac{f(y) - f(x)}{g(y) - g(x)} - \frac{f'(\xi)}{g'(\xi)}\right\vert}_{=0} + \left\vert \frac{f'(\xi)}{g'(\xi)} - \gamma\right\vert < \epsilon\quad\forall x,y\in(a,a+\delta),\;g(x)\neq g(y)
		\end{align*}
...
\begin{example}
	$\lim\limits_{x \to 0} \frac{2 - 2\cos x}{x^2} = 1$, denn es ist $\lim\limits_{x \to 0} \frac{(2 - 2\cos x)'}{(x^2)'} = \lim\limits_{x \to 0} \frac{2\sin x}{2x} \overset{\footnotesize\propref{mittelwertsatz_beispiel_sinx_x}}{=} 1$.
