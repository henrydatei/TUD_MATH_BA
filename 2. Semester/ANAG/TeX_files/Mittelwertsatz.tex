...
\begin{example}
	...
	\begin{tabularx}{\linewidth}{rX}
	$\xRightarrow{\propref{mittelwertsatz_ableitung_null_konstante_funktion}}$ & $f(x) = \mathrm{const}$ auf $(0,1)$ und $(2,3)$, aber auf jedem Intervall kann die Konstante anders sein.
	\end{tabularx}
\end{example}
...
\begin{proof}
	...
	\end{align*}
	$\xRightarrow{\propref{mittelwertsatz_schrankensatz}}$ $\vert \phi_k(1) - \phi_k(0)\vert = \vert f(a_k) - f(a_{k  - 1}) - f_{x_k} (a_{k  +1}) y_k \vert \le \sup\limits_{t\in (0,1)} \vert \phi_k'(\xi)\vert$, $k = 1,\dotsc,n$
