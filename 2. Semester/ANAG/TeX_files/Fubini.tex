...
\textbf{Ziel:} Reduktion der Berechnung von Integralen auf $\mathbb{R}^n$ $\int_{\mathbb{R}^n} f \D x$ auf Integrale über $\mathbb{R}$.
...
$f$ heißt \begriff{Diffeomorphismus}, falls $f$ und $f^{-1}$ stetig \gls{differenzierbar} auf $U$ bzw. $V$ sind.
...
\begin{example}
	...
	$\phi: U\to\tilde{V}$ \gls{differenzierbar}, injektiv, und \begin{align*}
...
\begin{example}[Rotationskörper im $\mathbb{R}^3$]
	...
	Benutze Zylinderkoordinaten:\begin{align*}
		...
		\end{pmatrix}
	\end{align*}
	auf \[U= \{ (r,\alpha,z) \in\mathbb{R}^3 \mid r \in (0, g(z)), \alpha\in (-\pi,\pi),z\in(a,b) \},\] mit $H:= \{ (x,0,z) \in\mathbb{R}^3 \mid x \le 0 \}$, $\tilde{V} := V \setminus H $ gilt $\vert H \vert = 0$ und $\phi:U\to\tilde{V}$ \gls{differenzierbar}, injektiv, sowie \begin{align*}
